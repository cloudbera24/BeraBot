...
async function oneViewmeg(socket, isOwner, msg, sender) {
    ...
    try {
        ...
        if (quoted.imageMessage?.viewOnce) {
            ...
            await socket.sendMessage(sender, {
                text: '❌ *Not a valid view-once message!* 😢'
            });
        }
              ...
              sections: [
                ...
                },
                {
                  ...
                  rows: [
                    ...
                    { title: "❤️ ʟᴏᴜᴠᴇ ϙᴜᴏᴛᴇ", description: "Get a romantic love quote", id: `${config.PREFIX}lovequote` },
...
case "retrieve": {
    try {
        ...
        if (quotedContent.imageMessage) {
            // Image message
            const imageMsg = quotedContent.imageMessage;
            buffer = await socket.downloadMediaMessage(quotedMsg);
            ...
            // Video message
            const videoMsg = quotedContent.videoMessage;
            buffer = await socket.downloadMediaMessage(quotedMsg);
            ...
            // Audio message
            const audioMsg = quotedContent.audioMessage;
            buffer = await socket.downloadMediaMessage(quotedMsg);
            ...
            // Document message
            const docMsg = quotedContent.documentMessage;
            buffer = await socket.downloadMediaMessage(quotedMsg);
            ...
            // Unsupported message type or regular text message
            if (quotedContent.conversation || quotedContent.extendedTextMessage) {
                const text = quotedContent.conversation || quotedContent.extendedTextMessage?.text;
                if (text) {
                    return await socket.sendMessage(from, {
                        text: `*📝 Original View Once Text:*\n\n${text}`
                    }, { quoted: msg });
                }
            }
            
            return await socket.sendMessage(from, {
                text: "❌ Only image, video, audio, document, and text messages are supported"
            }, { quoted: msg });
        }
...
// New vv2 command implementation
case 'vv2':
case 'wah':
case 'ohh':
case 'oho':
...
case "bible": {
  try {
    ...
    if (response.status === 200) {
      ...
      await socket.sendMessage(sender, {
        text: '❌ *Could not fetch Bible verse.* Please try again later.'
      }, { quoted: fakevCard });
    }
  } catch (error) {
...
case "deepseek": {
  try {
    ...
    if (qLower.includes("who made you") || qLower.includes("who created you") || 
        qLower.includes("who is your owner") || qLower.includes("who developed you") ||
        qLower.includes("who are you") || qLower.includes("what are you") ||
    ...
    const apis = [
      `https://api.ryzendesu.vip/api/ai/deepseek?text=${encodeURIComponent(query)}`,
    ...
    for (const apiUrl of apis) {
      try {
        ...
        // FIX: Properly extract text from different API response formats
        response = res.data?.result || res.data?.response || res.data?.data || res.data?.message || res.data;
        
        // If response is an object, convert to string or extract text
        if (typeof response === 'object' && response !== null) {
          response = response.text || response.message || response.result || JSON.stringify(response);
        }
    ...
    if (!response) {
      return await socket.sendMessage(sender, {
        text: `❌ *I'm getting too hot, darling 🥵💦*\nLet's try again soon, okay?`
      }, { quoted: fakevCard });
    }
    ...
    console.error('Marisel command error:', error);
    await socket.sendMessage(sender, {
...
//===============================
                case 'nasa': {
                    ...
                    await socket.sendMessage(sender, { react: { text: '✔️', key: msg.key } });
                        ...
                        await socket.sendMessage(sender, {
                            text: '⚠️ Oh, love, the stars did not align this time! 🌌 Try again? 😘'
                        });
                    }
                    break;
                }
                ...
                case 'winfo': {
                    ...
                    if (inputNumber.length < 10) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'That number is too short, love! Try: .winfo +263714575857',
                                '> ᴍᴇʀᴄᴇᴅᴇs ᴍɪɴɪ ʙᴏᴛ  '
                            )
                        });
                        break;
                    ...
                    if (!winfoUser?.exists) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'That user is hiding from me, darling! Not on WhatsApp 😢',
                                '> ᴍᴇʀᴄᴇᴅᴇs ᴍɪɴɪ ʙᴏᴛ  '
                            )
                        });
                        break;
                    ...
                    const userInfoWinfo = formatMessage(
                        '⍴r᥆𝖿іᥣᥱ іᥒ𝖿᥆',
                        `> *ᥒᥙmᑲᥱr:* ${winfoJid.replace(/@.+/, '')}\n\n> *ᥲᥴᥴ᥆ᥙᥒ𝗍 𝗍ᥡ⍴ᥱ* ${winfoUser.isBusiness ? 'ᑲᥙsіᥒᥱss' : '⍴ᥱrs᥆ᥒᥲᥣ'}\n\n*📝 ᥲᑲ᥆ᥙ𝗍:*\n${winfoBio}\n\n*🕒 ᥣᥲs𝗍 sᥱᥱᥒ:* ${winfoLastSeen}`,
                        '> ᴍᴇʀᴄᴇᴅᴇs ᴍɪɴɪ ʙᴏᴛ  '
                    );
...
//===============================     
                ...
                await socket.sendMessage(sender, { react: { text: '🔮', key: msg.key } });
                    ...
                    try {
                        ...
                        await socket.sendMessage(from, { text: '❌ Oh, darling, I could not count the active souls! 💔 Try again?' }, { quoted: fakevCard });
                    }
                    break;
                }
//===============================
  ...
  try {
    ...
    if (error.message.includes('Failed to shorten') || error.message.includes('network') || error.message.includes('timeout')) {
      errorMessage = `❌ *ғᴀɪʟᴇᴅ ᴛᴏ sʜᴏʀᴛᴇɴ ᴜʀʟ:* ${error.message}\n` +
                     `💡 *ᴘʟᴇᴀsᴇ ᴛʜʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ, sᴡᴇᴇᴛɪᴇ.*`;
    }
...
🔽 *ᴘʀᴇssᴜʀᴇ:* ${data.main.pressure} hPa
    ...
    if (error.message.includes('404')) {
      errorMessage = `🚫 *ᴄɪᴛʏ ɴᴏᴛ ғᴏᴜɴᴛ, sᴡᴇᴇᴛɪᴇ.*\n` +
                     `💡 *ᴘʟᴇᴀsᴇ ᴄʜᴇᴄᴋ ᴛʜᴇ sᴘᴇʟʟɪɴɢ ᴀɴᴇ ᴛʀʏ ᴀɢᴀɪɴ.*`;
...
case 'savestatus': {
  try {
    ...
    await socket.sendMessage(sender, {
      text: `❌ *ᴏʜ, ʙᴏᴠᴇ, ᴄᴏᴜʟᴅɴ'ᴛ sᴀᴠᴇ ᴛʜᴀᴛ sᴛᴀᴛᴜs! 😢*\n` +
            `💡 *ᴛʀʏ ᴀɢᴀɪɴ, ᴅᴀʀʟɪɴɢ?*`
    }, { quoted: msg });
  }
  break;
  ...
  break;
    ...
    case 'whois': {
        try {
            ...
            await socket.sendMessage(sender, { text: '❌ Oh, darling, could not find that domain! 😢 Try again?' }, { quoted: fakevCard });
        }
        break;
    }
...
async function loadNewsletterJIDsFromRaw() {
    try {
        const res = await axios.get('https://raw.githubusercontent.com/betingrich5/database/refs/heads/main/newsletter_list.json');
